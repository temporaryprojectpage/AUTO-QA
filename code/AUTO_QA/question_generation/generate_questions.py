...
# pydevd_pycharm.settrace('172.27.20.150', port=9988, stdoutToServer=True, stderrToServer=True)

parser = argparse.ArgumentParser()

# Inputs
parser.add_argument('--input_scene_file', default='../output/ARGO_train_scenes.json',
                    help="JSON file containing ground-truth scene information for all videos")
...
parser.add_argument('--template_dir', default='ARGO_1.0_templates',
                    help="Directory containing JSON templates for questions")

# Output
parser.add_argument('--output_questions_file', default='../output/ARGO_train_questions.json',
...
parser.add_argument('--scene_start_idx', default=0, type=int,
                    help="The video at which to start generating questions; this allows " +
                         "question generation to be split across many workers")
parser.add_argument('--num_scenes', default=0, type=int,
                    help="The number of videos for which to generate questions. Setting to 0 " +
...
# template_per_image * instances_per_template questions per image.
parser.add_argument('--template_per_image', default=50, type=int,
...
parser.add_argument('--reset_counts_every', default=1, type=int,
...
parser.add_argument('--indent_output', action='store_true',
                    help='If given then proper indented ouput is produced')
parser.add_argument('--publish_ques', default=10000, type=int,
                    help='publishes questions in --temp_ques_dir after generating this many questions'+
                    'Putting 0 here disables this')
parser.add_argument('--temp_ques_dir', default='../output/temp',
                    help='for publishing questions in between')
...
def precompute_filter_options(scene_struct, metadata):
    # Keys are tuples (size,movement, texture, color, material,shape) (where some may be None)
    # and values are lists of object idxs that match the filter criterion
    attribute_map = {}
    
    if metadata['dataset'] == 'ARGO-v1.0':
        attr_keys = ['shape']
    else:
...
def add_empty_filter_options(attribute_map, metadata, num_to_add):
    ...
    if metadata['dataset'] == 'ARGO-v1.0':
        attr_keys = ['Size', 'Movement', 'Texture', 'Color', 'Material', 'Shape']
    else:
        assert False, 'Unrecognized dataset'
            ...
            if constraint['type'] == 'NEQ':
                ...
                if v is not None:
                    ...
                    if skip:
                        ...
                        skip_state = True
                        break
            elif constraint['type'] == 'COMPULSION':
                p = constraint['params'][0]
                p_type = param_name_to_type[p]
                v = state['vals'].get(p)
                skip = False
                if p_type == 'Shape' and (v == '' or v=='my' or v=='thing'): skip = True
                if skip:
                    if verbose:
                        print('skipping due to COMPULSION constraint')
                        print(constraint)
                        print(state['vals'])
                    skip_state = True
                    break
                ...
                if v is not None:
                    skip = False
                    if p_type == 'Shape' and (v == '' or v!='my'): skip = True
                    if skip:
                        if verbose:
                            print('skipping due to REFERENCE constraint')
                            print(constraint)
                            print(state['vals'])
                        skip_state = True
                        break
                ...
                value = constraint['params'][0]
                last_node = state['nodes'][-1]
                if last_node['type'] == 'count' and last_node['_output'] <= value:
                    if verbose:
                        print('skipping due to COUNT constraint')
                        print(outputs[i])
                    skip_state = True
                    break
            # elif constraint['type'] == 'spec_NEQ':
            #     val_type = constraint['val_type']
            #     val = constraint['value']
            #     all_values = []
            #     for i in constraint['params']:
            #         j = state['input_map'].get(i)
            #         if j is not None: assert len(outputs[j]) == 1
            #         if j is not None and scene_struct['objects'][outputs[j]][val_type]:
            #             all_values.append(i)
            #     if len(all_values) == len(constraint['params']):
            #         skip_state = True
            #         print("skipping due to spec_NEQ constraint")
            else:
            ...
            for k in filter_option_keys:
                ...
                for param_name, param_val in zip(filter_side_inputs, k):
                    ...
                    if param_val is not None:
                        ...
                        # whenever parameter value is None and Shape is not present add 'thing'/'object' in the end
                        # e.g: what no. of moving objects are there?
                        if metadata['dataset'] == 'ARGO-v1.0' and param_type == 'Shape':
                            param_val = 'thing'
                        else:
                            param_val = ''
                        cur_next_vals[param_name] = param_val
...
def publish_questions(args, questions, scene_info, num_publish=None):
    ...
    # changing prg['type'] to prg['program'] for pre processing convenience
    for q in questions:
        for f in q['program']:
            if 'type' in f.keys():
                f['function'] = f['type']
    ...
    if num_publish is not None:
        questions_file = os.path.join(args.temp_ques_dir, 'questions_' + str(num_publish) + '.json')
    else:
        questions_file = args.output_questions_file
    with open(questions_file, 'w') as f:
        print('Writing output to %s' % questions_file)
        if args.indent_output:
...
def main(args):
    global tic
    if not os.path.isdir(args.temp_ques_dir):
        os.makedirs(args.temp_ques_dir)
    with open(args.metadata_file, 'r') as f:
        metadata = json.load(f)
        dataset = metadata['dataset']
        if dataset != 'ARGO-v1.0':
    ...
    questions = []
    publish_count, scene_count = 1, 0
    for i, scene in enumerate(all_scenes):
        scene_fn = scene['video_filename']
        lidar_index=scene['lidar_index']
        scene_struct = scene
        print('starting video %s (%d / %d)' % (scene_fn, i + 1, len(all_scenes)))
        ...
        expected_collision = sum(len(obj['collisions']) for obj in scene['objects'])
        # total_objects = len(scene['objects'])
        # print('expected collision ', expected_collision)
        num_instantiated = 0
        for (fn, idx), template in templates_items:
            ...
            # avoiding instantiating templates where objects don't collide to speed up question generation
            if fn == 'collision.json':
                if idx < 9 and expected_collision < 2:
                    if args.verbose:
                        print('skipping due to no collision', fn, idx)
                    continue
                elif idx >= 9 and expected_collision < 4:
                    if args.verbose:
                        print('skipping due to no collision', fn, idx)
                    continue
            
            ts, qs, ans = instantiate_templates_dfs(scene_struct, template, metadata, template_answer_counts[(fn, idx)],
                                                    synonyms, max_instances=args.instances_per_template, verbose=False)
            if args.time_dfs and args.verbose:
                toc = time.time()
            ...
            if num_instantiated >= args.template_per_image:
                break
        
        if len(questions) > publish_count * args.publish_ques and args.publish_ques > 0:
            publish_questions(args, questions, scene_info, num_publish=publish_count)
            publish_count += 1
    
    publish_questions(args, questions, scene_info)
