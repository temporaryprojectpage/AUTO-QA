...
def specific_filter_handler(scene_struct, inputs, side_inputs):
    """
    ...
    sides = []
    for values in side_inputs:
        sides.append(values.split('_'))
    for idx in inputs[0]:
        for attribute, value in sides:
            atr = scene_struct['objects'][idx][attribute]
            if value == atr or value in atr:
                if idx in output:
                    output.remove(idx)
...
def possible_collision_handler(scene_struct, inputs, side_inputs):
    assert len(inputs) == 1
    assert len(side_inputs) == 0
    idx = inputs[0]
    obj = scene_struct['objects'][idx]
    assert 'collisions' in obj
...
#To query the closest object
def closest_handler(scene_struct,inputs,side_inputs):
    assert len(inputs)>=1
    assert len(side_inputs)==0
    print(inputs,type(inputs[0]))
    
    distance=[np.mean(scene_struct['objects'][i]['coordinates'],axis=0)[0]**2+np.mean(scene_struct['objects'][i]['coordinates'],axis=0)[1]**2 for i in inputs[0]]
    min_value = min(distance)
    min_index = distance.index(min_value)
    
    return scene_struct['objects'][inputs[0][min_index]]['shape']
...
execute_handlers = {
    ...
    'filter_texture': make_filter_handler('texture'),
    ...
    'query_texture':make_query_handler('texture'),
    'query_movement':make_query_handler('movement'),
    'query_speed': make_query_handler('speed'),
    'exist': exist_handler,
    ...
    'equal_texture': equal_handler,
    'equal_speed':equal_handler,
    ...
    'same_texture': make_same_attr_handler('texture'),
    'same_movement': make_same_attr_handler('movement'),
    'same_speed': make_same_attr_handler('speed'),
    'specific_filter': specific_filter_handler,
    'possible_collisions': possible_collision_handler,
    'closest':closest_handler
}
